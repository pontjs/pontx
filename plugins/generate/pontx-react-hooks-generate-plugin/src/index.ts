import { InnerOriginConfig, PontGeneratorPlugin, PontManager } from "pontx-manager";
import { PontAPI, PontSpec } from "pontx-spec";
...
async function getBuiltinStructure() {
  ...
  const promises = dir.map(async (filename) => {
    const content = await fs.readFile(path.join(__dirname, "../builtin", filename), "utf8");
...
${super.generateSpecIndexTsCode(spec)}`;
  ...
  generateSpecIndexJsCode(spec: PontSpec): string {
    if (spec.name) {
      return super.generateSpecIndexJsCode(spec);
    }
...
class PontReactHooksGeneratorPlugin extends PontGeneratorPlugin {
  static async generateSingleSpec(pontSpec: PontSpec, basePath: string) {
    ...
    return FileGenerator.generateSdk(fileGenerator);
  }
